...
class P2PNetwork:
    ...
    def validate(self):
        ...
        for node in self.nodes.values():
            ...
            if degree < self.min_neighbors:
                raise ValueError(f"{node.id} possui poucos vizinhos")

            if degree > self.max_neighbors:
                raise ValueError(f"{node.id} possui muitos vizinhos")

            if not node.resources:
                raise ValueError(f"{node.id} não possui recursos")
    ...
    def random_walk(self, start_node, resource_id, ttl):
        ...
        while ttl >= 0:
            ...
            current = random.choice(list(node.neighbors))
            involved.add(current)
            messages += 1
            ttl -= 1
